...
4. `admin/wrangler.jsonc` 管理 D1 绑定和固定 Origin；生产项目通过 Secret `ADMIN_EMAIL` 指定唯一管理员的数字 QQ 邮箱（密码登录必需，不再只是显示名称）。现有管理员先执行 `0003_password_auth.sql`，再上线并在“数据与备份 → 账户与网站密码”验证已有密钥、设置专用密码。全新项目仍以一次性初始化凭据绑定首把密钥后再设置密码；凭据、密码和恢复码不得提交仓库或发到聊天里。
...
网站密码采用服务器原生异步 PBKDF2-HMAC-SHA256、600,000 次迭代和每次随机生成的 32 字节盐；数据库仅保存散列。要求 15–128 字符，密码尝试每 IP 每 15 分钟最多 5 次、全局最多 20 次，成功也计入额度。改密撤销其他会话；恢复码可重设密码并撤销旧设备、会话和旧恢复码，私人记录保留。不提供邮件验证码或 QQ 邮箱密码验证。免费运行额度耗尽时拒绝访问，不降低密码散列强度或跳过认证。
