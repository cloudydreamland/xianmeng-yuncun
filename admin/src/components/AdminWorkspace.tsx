...
import { authApi } from './PasskeyAuth';
import { PasswordSecurity } from './PasswordAuth';
...
export default function AdminWorkspace({ publicOrigin }: { publicOrigin: string }) {
  ...
  return <div className="admin-shell">
    <header className="admin-header"><div><p className="kicker">PRIVATE CLOUD DESK · 私人云案</p><h1>我的工作台</h1><p>只有唯一管理员能够查看和修改，记录直接保存到私人云端。</p></div><div className="account"><span>{sessionEmail} · 管理员已验证</span><button className="text-button" disabled={busy} onClick={() => void logout()}>退出登录</button></div></header>
