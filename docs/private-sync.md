...
> **历史方案，仅用于旧数据迁移参考。** 当前工作台使用指定 QQ 邮箱、网站专用密码和 D1 记录级存储；见 [管理端安全说明](ADMIN_SECURITY.md)。旧 `/api/sync` 写入与删除接口已关闭，只保留经访问密钥验证的读取，以便迁移旧加密快照。不要按下文为新工作台启用旧同步方案；旧数据迁移确认前不要删除旧数据库或访问配置。
