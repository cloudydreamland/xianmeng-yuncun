...
test('首页计划摘要提供公开推进入口', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(summary.getByRole('link', { name: '完整推进台 →' })).toHaveAttribute('href', '/world/moon-pool/#plan-dashboard-title');
});
...
test('公开月潭即使存在旧私人数据也不渲染编辑器', async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem('yuncun-local-plans-v1', JSON.stringify([{ id: 'private-plan', title: '仅本人可见' }])));
  await page.goto('/world/moon-pool/');
  await expect(page.locator('[data-local-planner],[data-life-inbox]')).toHaveCount(0);
  await expect(page.getByText('仅本人可见')).toHaveCount(0);
});
